package zov.polyak.util.server;
...
import zov.polyak.module.list.misc.ScoreboardHealth;
...
public class Server implements IMinecraft {
    ...
    public float getHealth(LivingEntity entity, boolean gapple) {
        if (Polyak.getInstance().getModuleStorage().get(ScoreboardHealth.class).isEnabled()) {
