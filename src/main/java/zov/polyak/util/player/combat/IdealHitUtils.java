package zov.polyak.util.player.combat;
...
import zov.polyak.util.player.other.WorldUtils;
...
public class IdealHitUtils implements IMinecraft {
    ...
    public boolean canAIFall() {
        return ((getBlock(0, 3, 0) == Blocks.AIR && getBlock(0, 2, 0) == Blocks.AIR && getBlock(0, 1, 0) == Blocks.AIR)
                || Polyak.getInstance().getServerManager().getFallDistance() < (getBlock(0, 2, 0) != Blocks.AIR ? 0.08f : 0.6f)
                || Polyak.getInstance().getServerManager().getFallDistance() > 1.2f);
    }
    ...
    public boolean canCritical() {
        ...
        boolean notCrit = mc.player.isInLava()
                ...
                || Polyak.getInstance().getModuleStorage().get(FreeCamera.class).isEnabled()
                || mc.player.isOnGround() && !mc.options.jumpKey.isPressed() && !Polyak.getInstance().getModuleStorage().get(KillAura.class).onlySpace.getValue();
