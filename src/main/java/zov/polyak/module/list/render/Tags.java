...
import zov.polyak.util.party.PartyPlayerPos;
...
import zov.polyak.util.render.math.ProjectionUtil;
import zov.polyak.util.render.renderers.IRenderer;
import zov.polyak.util.render.msdf.Fonts;
import zov.polyak.util.render.msdf.MsdfFont;
...
public class Tags extends Module {
    ...
    public static Text processName(PlayerEntity entity) {
        ...
        if (Polyak.getInstance().getModuleStorage().get(NameProtect.class).isEnabled() && mc.player != null) {
            String scoreName = entity.getNameForScoreboard();
            String myName = mc.player.getNameForScoreboard();
            String customName = Polyak.getInstance().getModuleStorage().get(NameProtect.class).getCustomName();

            if (scoreName.equals(myName) || entity.getUuid().equals(mc.player.getUuid())) {
                name = ReplaceUtil.replace(name, myName, customName);
            } else if (Polyak.getInstance().getModuleStorage().get(NameProtect.class).hideFriends.getValue() && FriendRepository.isFriend(scoreName)) {
    ...
    private void renderPlayerTags(MsdfFont font, float tickDelta, EventHUD e) {
        ...
        for (PlayerEntity entity : worldPlayers) {
            ...
            int currentHp = (int) zov.polyak.util.server.Server.getHealth(entity, false);
            ...
            Interface iface = Polyak.getInstance().getModuleStorage().get(Interface.class);
            if (iface != null) {
