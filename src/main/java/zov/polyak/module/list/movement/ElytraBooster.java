...
import zov.polyak.event.list.FireworkEvent;
...
public class ElytraBooster extends Module {
    ...
    private void onFirework(FireworkEvent event) {
        ...
        KillAura aura = Polyak.getInstance().getModuleStorage().get(KillAura.class);
        LivingEntity target = aura.getTarget();

        if (aura != null && aura.isEnabled() && aura.elytraSlowdown.getValue()) {
            if (target != null) {
                if (aura.slowdownMode.is("По радиусу")) {
                    Vec3d predictedPos = PredictUtils.predict(target, aura.predictValue.getValue());
                    double dist = mc.player.getEyePos().distanceTo(predictedPos);
                    double radius = aura.slowdownRadius.getValue();

                    if (dist < radius) {
                        double ratio = MathHelper.clamp(dist / radius, 0.0, 1.0);
                        double smoothCurve = ratio * ratio * (3.0 - 2.0 * ratio);
                        float min = aura.minSpeed.getFloatValue();
                        speed *= (float) (min + (1.0f - min) * smoothCurve);
                    }
                } else if (aura.slowdownMode.is("Перед ударом") && KillAura.isSlowdownActive) {
                    speed = 1.3f;
                }
            }
        }
