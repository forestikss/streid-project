...
import zov.polyak.module.list.combat.*;
import zov.polyak.module.list.misc.*;
import zov.polyak.module.list.movement.*;
import zov.polyak.module.list.player.*;
import zov.polyak.module.list.render.*;
import zov.polyak.module.list.render.hud.Interface;
import zov.polyak.module.settings.BooleanSetting;
import zov.polyak.module.settings.ThemeSetting;
import zov.polyak.util.IMinecraft;
import zov.polyak.util.base.Instance;
import zov.polyak.util.party.connection.PartyApiClient;
...
public class ModuleStorage implements IMinecraft {
    ...
    public void injectRegisterModules() {
        modules.addAll(List.of(
                ...
                new FreeCamera(), new SwingAnimations(), new Predictions(),
                new DogFly(), new AutoTpaccept(), new RPSpoofer(), new FireFly(),
                new AntiBot(), new AutoExplosion(), new DeathCoords(), new CrystalOptimizer(),
                new GuiMove(), new Tracers(), new ElytraMotion(), new Velocity(), new ElytraFlight(),
                new ViewModel(), new KillEffect(), new AutoArmor(), new LonyHelper(),new Speed(),
                new AutoTool(), new TapeMouse(), new Ambience(), new BlockOverlay(), new FreeLook(), new Chams(),
                new Trails(), new FastExp(), new NameProtect(), new CrystalSpammer(), new ChinaHat(),
                new AirStuck(), new AutoSwap(), new NoSlow(), new NoWeb(), new FakePlayer(), new Interface(), new AutoEat(),new AutoLeave(), new WellJoiner(), new WellSaver(),
                new Wings(), new ScoreboardHealth(), new AutoPotion(), new GrimGlide()
        ));
